...
void GinsPreInteg::AddGnss(const GNSS& gnss, bool with_odom)
{
    if (with_odom)
    {
        if (last_data_sign_ == odom_)
        {
            last_data_sign_ = gnss_;
            return;
        }
        last_data_sign_ = gnss_;
...
void GinsPreInteg::AddOdom(const sad::Odom& odom, bool with_odom)
{
    if (with_odom)
    {
        if (last_data_sign_ == gnss_)
        {
            last_data_sign_ = odom_;
            return;
        }
        ...
        // LOG(INFO) << odom.left_pulse_ << " " << odom.right_pulse_ << " " << std::fixed << std::setprecision(10) << odom.timestamp_;
...
void GinsPreInteg::Optimize(const int now_data_sign)
{
